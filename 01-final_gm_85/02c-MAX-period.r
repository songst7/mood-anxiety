...
dataTable <- select(df, Subj, ROI, cond, response, TRAIT, STATE, Phase)

# Transfer data into contrast coding as response_threat - response_safe

dataTable <- dataTable %>% left_join(dataTable, by = c("Subj", "ROI", "Phase", "TRAIT", "STATE"))
dataTable <- dataTable %>% filter(cond.x == -0.5, cond.y == 0.5)
dataTable <- dataTable %>% mutate(diff = response.y - response.x)
dataTable <- dataTable %>% mutate(phase = ifelse(Phase == "early", -0.5, 0.5))
dataTable <- dataTable %>% select(Subj, ROI, phase, diff, TRAIT, STATE)
dataTable %>% head()
...
mod <- '1 + phase + TRAIT + STATE'
modelForm <- paste0('diff ~ ', mod, ' + (', mod, ' | gr(ROI, dist = "student")) + (1 + phase| gr(Subj, dist = "student"))')
...
priorRBA$prior[8:9] <- "gamma(3.325, 0.2)"
priorRBA$prior[11] <- "gamma(3.325, 0.2)"

print(priorRBA)

# Create a result directory
if (!dir.exists("results_offset/phase")){dir.create("results_offset/phase")}
...
cat(stan_code, file = paste0("results_offset/model2/", project, "_only_stancode.stan"), sep='\n')
...
save.image(file = paste0("results_offset/model2/", project, ".RData"))
